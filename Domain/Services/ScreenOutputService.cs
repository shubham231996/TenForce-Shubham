...
namespace Test_Taste_Console_Application.Domain.Services
{
    ...
    public class ScreenOutputService : IOutputService
    {
        ...
        public void OutputAllPlanetsAndTheirAverageMoonGravityToConsole()
        {
                ...
                foreach (Planet planet in planets)
                {
                    ...
                    if (planet.HasMoons())
                    {
                        if (planet.Moons != null && planet.Moons.Count > 0)
                        {
                            ...
                            {
                                float massValue = planet.Moons.ElementAt(i).MassValue;        //get mass value
                                float massExponenet = planet.Moons.ElementAt(i).MassExponent; //get mass exponen
                                if (massValue > 0 && massExponenet > 0)
                                    divide += massValue / massExponenet;
                            }
                            result = Math.Round(divide / planet.Moons.Count, 2);
                        }

                        ConsoleWriter.CreateText(new string[] { $"{planet.Id}", $"{result}" }, columnSizes);
                    }
                    else
                    {
            ...
            {
                Console.Clear();
                textToEnter = "Something want wrong please try again after some time";
                Console.WriteLine(String.Format("{0," + ((Console.WindowWidth / 2) + (textToEnter.Length / 2)) + "}", textToEnter));
            }
